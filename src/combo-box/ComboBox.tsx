...
import type { ComboBoxConfig, ComboBoxSize } from "../theme/types";
import {
  buildLinearGradient,
  buildConicGradient,
  buildInkPath,
...
export const defaultComboBoxConfig: ComboBoxConfig =
  defaultTheme.components.comboBox;

// ---------------------------------------------------------------------------
// Config context — consumed by ComboBoxItem / ComboBoxSection
// ---------------------------------------------------------------------------

const ComboBoxConfigContext = React.createContext<ComboBoxConfig>(
...
export type ComboBoxItemProps = Omit<ListBoxItemProps, "className" | "children"> & {
  children: React.ReactNode;
};

/** A styled option inside a <ComboBox>. Uses config from context. */
export function ComboBoxItem({ children, ...props }: ComboBoxItemProps) {
  const config = React.useContext(ComboBoxConfigContext);
  const text =
...
/** A labelled group of options inside a <ComboBox>. */
export function ComboBoxSection({ header, children }: ComboBoxSectionProps) {
  const config = React.useContext(ComboBoxConfigContext);
  return (
    <Section>
      {header != null && (
...
export type ComboBoxProps = {
  ...
  size?: ComboBoxSize;
  /**
   * Icon rendered before the input (inside the border). Absolute-positioned;
   * the input will have `withPrefix` padding applied when this is present.
   */
  prefix?: React.ReactNode;
  /** Option items. Pass <ComboBoxItem> children. */
  children?: React.ReactNode;
...
} & Omit<AriaComboBoxProps<object>, "className" | "style" | "children">;
...
export function ComboBox({
  ...
  // Manual focus-within tracking. The ComboBox contains an Input and a Button;
  // both can hold focus. The contains() check keeps the focused state while
        ...
        >
          ...
          <div
            className={cn(
              ...
              corners && config.corners &&
                "group-data-[focused]:!border-transparent group-data-[focused]:!ring-0 group-data-[focused]:!shadow-none group-data-[open]:!border-transparent group-data-[open]:!ring-0",
            )}
          ...
          >
            ...
            <Button className={cn(config.button, sizeConfig.buttonWidth)}>
              <ChevronDownIcon />
            </Button>
          </div>
        </div>
